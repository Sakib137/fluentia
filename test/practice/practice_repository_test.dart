...
void main() {
  group('SqlitePracticeRepository Tests', () {
    ...
    test('Retrieves activities filtered by skill', () async {
      final speakingActivities = await repository.getActivitiesBySkill(PracticeSkill.speaking);
      expect(speakingActivities, isNotEmpty);
      expect(speakingActivities.every((a) => a.skill == PracticeSkill.speaking), isTrue);

      final writingActivities = await repository.getActivitiesBySkill(PracticeSkill.writing);
      expect(writingActivities, isNotEmpty);
      expect(writingActivities.every((a) => a.skill == PracticeSkill.writing), isTrue);
    });
    ...
    test('Selects recommended activity based on goals and level', () async {
      ...
      expect(recommended.skill, anyOf(PracticeSkill.speaking, PracticeSkill.writing));
    });

    test('Saving completed session updates PracticeSessions and UserProgress tables', () async {
      ...
      final todayMinutes = await repository.getTodayPracticeMinutes(date: now);
      expect(todayMinutes, 5);
    });

    test('Saving abandoned session logs session without adding practice minutes', () async {
